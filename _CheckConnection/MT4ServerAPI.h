...
struct PluginInfo
  {
   ...
   int               reserved[32];          // reserved
	...
	PluginInfo() { ZeroMemory(this, sizeof(PluginInfo)); }
	PluginInfo(const char* _name, unsigned int _version, const char* _copyright)
	{
		ZeroMemory(this, sizeof(PluginInfo));
		strcpy(name, _name);
		version = _version;
		strcpy(copyright, _copyright);
	}
  };
//+------------------------------------------------------------------+
...
struct PluginCfg
  {
   ...
   int               reserved[16];
	...
	PluginCfg() { ZeroMemory(this, sizeof(PluginCfg)); }
  };
//+------------------------------------------------------------------+
...
struct ConAccess
  {
   ...
   int               reserved[17];          // reserved
	...
	ConAccess() { ZeroMemory(this, sizeof(ConAccess)); }
  };
...
struct ConCommon
  {
   ...
   int               reserved[53];
	...
	ConCommon() { ZeroMemory(this, sizeof(ConCommon)); }
  };
//+------------------------------------------------------------------+
//| Time config                                                      |
//+------------------------------------------------------------------+
struct ConTime
  {
   ...
   int               reserved[3];           // reserved
	...
	ConTime() { ZeroMemory(this, sizeof(ConTime)); }
  };
//+------------------------------------------------------------------+
...
struct ConBackup
  {
   ...
   time_t            watch_timestamp;        // watch dog timestamp
	...
	ConBackup() { ZeroMemory(this, sizeof(ConBackup)); }
  };
...
struct ConFeeder
  {
   ...
   int               unused[33];            // reserved
	...
	ConFeeder() { ZeroMemory(this, sizeof(ConFeeder)); }
  };
...
struct ConGroupSec
  {
   ...
   int               reserved[3];
	...
	ConGroupSec() { ZeroMemory(this, sizeof(ConGroupSec)); }
  };
//+------------------------------------------------------------------+
...
struct ConGroupMargin
  {
   ...
   int               reserved[7];
	...
	ConGroupMargin() { ZeroMemory(this, sizeof(ConGroupMargin)); }
  };
//---- dealing mode
...
struct ConGroup
  {
   ...
   int               reserved[17];
	...
	ConGroup() { ZeroMemory(this, sizeof(ConGroup)); }
  };
//--- margin mode
...
struct ConHoliday
  {
   ...
   ConHoliday       *next;                        // internal data
	...
	ConHoliday() { ZeroMemory(this, sizeof(ConHoliday)); }
  };
//+------------------------------------------------------------------+
...
struct LiveInfoFile
  {
   ...
   int               reserved[10];                // reserved
	...
	LiveInfoFile() { ZeroMemory(this, sizeof(LiveInfoFile)); }
  };
//----
struct ConLiveUpdate
  {
   ...
   ConLiveUpdate    *next;                        // internal data
	...
	ConLiveUpdate() { ZeroMemory(this, sizeof(ConLiveUpdate)); }
  };
//---- LiveUpdate type
...
struct ConManagerSec
  {
   ...
   int               unused[16];                  // reserved
	...
	ConManagerSec() { ZeroMemory(this, sizeof(ConManagerSec)); }
  };
//+------------------------------------------------------------------+
...
struct ConManager
  {
   ...
   int               reserved[22];
	...
	ConManager() { ZeroMemory(this, sizeof(ConManager)); }
  };
//+------------------------------------------------------------------+
...
struct ConSession
  {
   ...
   short             align[7];                    // internal data
	...
	ConSession() { ZeroMemory(this, sizeof(ConSession)); }
  };
//----
struct ConSessions
  {
   ...
   int               reserved[2];                 // reserved
	...
	ConSessions() { ZeroMemory(this, sizeof(ConSessions)); }
  };
//+------------------------------------------------------------------+
//| Security config                                                  |
//+------------------------------------------------------------------+
...
struct ConSymbol
  {
   ...
   int               unused[22];             	  // reserved
	...
	ConSymbol() { ZeroMemory(this, sizeof(ConSymbol)); }
  };
//+------------------------------------------------------------------+
//| Symbols enumeration                                              |
//+------------------------------------------------------------------+
...
struct ConSymbolGroup
  {
   ...
   char              description[64];             // group description
	...
	ConSymbolGroup() { ZeroMemory(this, sizeof(ConSymbolGroup)); }
  };
//+------------------------------------------------------------------+
...
struct FeedTick
  {
   ...
   char              reverved[8];                // reserved
	...
	FeedTick() { ZeroMemory(this, sizeof(FeedTick)); }
  };
//+------------------------------------------------------------------+
//| Feeder data quote                                                |
//+------------------------------------------------------------------+
struct FeedData
  {
   ...
   char              reserved[64];               // reserved
	...
	FeedData() { ZeroMemory(this, sizeof(FeedData)); }
  };
//+------------------------------------------------------------------+
...
struct LogInfo
  {
   ...
   int               unused[2];                  // reserved
	...
	LogInfo() { ZeroMemory(this, sizeof(LogInfo)); }
  };
//---
#pragma pack(pop)
//--- error code
...
struct LogRequest
  {
   ...
   char              filter[64];                 // filter
	...
	LogRequest() { ZeroMemory(this, sizeof(LogRequest)); }
  };
//--- log request type
...
struct PerformanceInfo
  {
   ...
   int               sockets;                    // reserved
	...
	PerformanceInfo() { ZeroMemory(this, sizeof(PerformanceInfo)); }
  };
#pragma pack(pop)
//+------------------------------------------------------------------+
...
struct UserRecord
  {
   ...
   char              api_data[16];               // for APIs usage
	...
	UserRecord() { ZeroMemory(this, sizeof(UserRecord)); }
  };
//+------------------------------------------------------------------+
...
struct UserInfo
  {
   ...
   ConGroup          grp;                        // client group
	...
	UserInfo() { ZeroMemory(this, sizeof(UserInfo)); }
  };
//+------------------------------------------------------------------+
//| Rate history record                                              |
//+------------------------------------------------------------------+
...
struct RateInfo
  {
   ...
   double            vol;                       // volume
	...
	RateInfo() { ZeroMemory(this, sizeof(RateInfo)); }
  };
#pragma pack(pop)
//+------------------------------------------------------------------+
//| Tick structure                                                   |
...
struct TickAPI
  {
   ...
   double            bid,ask;                    // normalized bid & ask
	...
	TickAPI() { ZeroMemory(this, sizeof(TickAPI)); }
  };
//+------------------------------------------------------------------+
...
struct MailBoxHeader
  {
   ...
   int               reserved[1];                // reserved
	...
	MailBoxHeader() { ZeroMemory(this, sizeof(MailBoxHeader)); }
  };
//+------------------------------------------------------------------+
...
struct TradeTransInfo
  {
   ...
   int               crc;                        // CRC
	...
	TradeTransInfo() { ZeroMemory(this, sizeof(TradeTransInfo)); }
  };
#pragma pack(pop)
...
struct RequestInfo
  {
   ...
   RequestInfo      *prev,*next;                 // internal data
	...
	RequestInfo() { ZeroMemory(this, sizeof(RequestInfo)); }
  };
//---- request status
...
struct TradeRecord
  {
   ...
   TradeRecord      *next;                       // internal data
	...
	TradeRecord() { ZeroMemory(this, sizeof(TradeRecord)); }
  };
#pragma pack(pop)
//+------------------------------------------------------------------+
...
struct MarginLevel
  {
   ...
   double            equity;                     // equity
	...
	MarginLevel() { ZeroMemory(this, sizeof(MarginLevel)); }
  };
//+------------------------------------------------------------------+
...
struct OverNightData
  {
   ...
   int               trades_max;                 // internal data
	...
	OverNightData() { ZeroMemory(this, sizeof(OverNightData)); }
  };
//+------------------------------------------------------------------+
...
struct DailyReport
  {
   ...
   int               reserved[3];
	...
	DailyReport() { ZeroMemory(this, sizeof(DailyReport)); }
  };
//+------------------------------------------------------------------+
...
struct StateReport
  {
   ...
   double            margin_free;                // free margin
	...
	StateReport() { ZeroMemory(this, sizeof(StateReport)); }
  };
//+------------------------------------------------------------------+
...
struct GroupState
  {
   ...
   int               margins_total;
	...
	GroupState() { ZeroMemory(this, sizeof(GroupState)); }
  };
//+------------------------------------------------------------------+
//| Server access class                                              |
//+------------------------------------------------------------------+
